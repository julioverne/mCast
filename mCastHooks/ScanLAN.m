...
- (void)startScan {
    ...
    if ([self isIpAddressValid:self.localAddress] && (a.count == 4) && (b.count == 4)) {
        ...
        self.timer = [NSTimer scheduledTimerWithTimeInterval:0.3f target:self selector:@selector(pingAddress) userInfo:nil repeats:YES];
    }
}
...
- (void)pingResult:(NSNumber*)success {
    ...
    if (success.boolValue) {
        NSString *deviceIPAddress = [[[[NSString stringWithFormat:@"%@%d", self.baseAddress, (int)self.currentHostAddress] stringByReplacingOccurrencesOfString:@".0" withString:@"."] stringByReplacingOccurrencesOfString:@".00" withString:@"."] stringByReplacingOccurrencesOfString:@".." withString:@".0."];
